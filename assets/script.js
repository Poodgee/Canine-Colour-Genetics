...
function buildParentsUI() {
  ...
  parents.forEach(parent => {
    ...
    order.forEach(locus => {
      ...
      row.appendChild(lbl);
      const s1 = document.createElement('select'); s1.id = `${parent}-${locus}-a1`;
      const s2 = document.createElement('select'); s2.id = `${parent}-${locus}-a2`;
      data.loci[locus].alleles.forEach(a => {
        s1.add(new Option(a.id, a.id)); s2.add(new Option(a.id, a.id));
      });
...
function resetHandler() {
  const url = window.location.href.toLowerCase();
  let defaults = {};
  if (url.includes('yakutian')) defaults = { 'K': ['ky', 'ky'], 'E': ['E', 'E'], 'A': ['a', 'a'], 'B': ['B', 'B'], 'D': ['D', 'D'], 'S': ['S', 'sp'] };
  else if (url.includes('greatdane')) defaults = { 'M': ['m', 'm'], 'H': ['h', 'h'], 'K': ['KB', 'KB'], 'E': ['Em', 'Em'], 'A': ['ay', 'ay'], 'D': ['D', 'D'], 'S': ['S', 'S'] };
...
function resolvePhenotypes(punnett) {
  ...
  return Object.entries(map).map(([k, v]) => {
    ...
    return { name, carrier, genoStr, prob: v, isStandard: phIsStandard(name) };
...
function determinePhenotype(geno) {
  ...
  if (url.includes('yakutian')) {
    ...
    const bGeno = safeGet(geno, 'B'), dGeno = safeGet(geno, 'D'), isLiver = bGeno === 'bb', isBlue = dGeno === 'dd', isIsabella = isLiver && isBlue;
    ...
    const mGeno = safeGet(geno, 'M'), hGeno = safeGet(geno, 'H'), kGeno = safeGet(geno, 'K'), aGeno = safeGet(geno, 'A'), dGeno = safeGet(geno, 'D'), sGeno = safeGet(geno, 'S');
...
function formatGeno(geno) {
  ...
  return order.filter(locus => geno[locus]).map(locus => geno[locus]).join(' ');
}
...
function addLayer(container, src) {
  ...
  img.style.width = '100%'; img.style.height = '100%'; img.style.pointerEvents = 'auto';
...
function drawPie(items) {
  ...
  items.forEach((it, i) => {
    ...
    ctx.beginPath(); ctx.moveTo(cx, cy); ctx.arc(cx, cy, r, start, start + sliceAngle);
